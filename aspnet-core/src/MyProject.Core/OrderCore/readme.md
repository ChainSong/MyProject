...
```csharp
Configuration.Authorization.Providers.Add<WMS_OrderAllocationAuthorizationProvider>();
...
```csharp
public virtual DbSet<WMS_OrderAllocation>  WMS_OrderAllocations { get; set; }
...
{
   modelBuilder.ApplyConfiguration(new  WMS_OrderAllocationMapper());
}
...
```csharp 
        ...
        protected void SetUserTablePrefix(ModelBuilder modelBuilder)
        {
            // 代码生成器生成的表实体配置全部添加到这里
             modelBuilder.ApplyConfiguration(new  WMS_OrderAllocationMapper());  // 请将代码配置到这里 
        }
...
```
Add-Migration Add_WMS_OrderAllocationEntity_Migration
```
