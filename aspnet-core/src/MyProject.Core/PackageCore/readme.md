...
```csharp
Configuration.Authorization.Providers.Add<WMS_PackageAuthorizationProvider>();
...
```csharp
public virtual DbSet<WMS_Package>  WMS_Packages { get; set; }
...
{
   modelBuilder.ApplyConfiguration(new  WMS_PackageMapper());
}
...
```csharp 
        ...
        protected void SetUserTablePrefix(ModelBuilder modelBuilder)
        {
            // 代码生成器生成的表实体配置全部添加到这里
             modelBuilder.ApplyConfiguration(new  WMS_PackageMapper());  // 请将代码配置到这里 
        }
...
```
Add-Migration Add_WMS_PackageEntity_Migration
```
