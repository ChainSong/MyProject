...
```csharp
public virtual DbSet<WMS_ReceiptDetail>  WMS_ReceiptDetails { get; set; }
...
{
   modelBuilder.ApplyConfiguration(new  WMS_ReceiptDetailMapper());
}
...
```csharp 
        ...
        protected void SetUserTablePrefix(ModelBuilder modelBuilder)
        {
            // 代码生成器生成的表实体配置全部添加到这里
             modelBuilder.ApplyConfiguration(new  WMS_ReceiptDetailMapper());  // 请将代码配置到这里 
        }
...
```
Add-Migration Add_WMS_ReceiptDetailEntity_Migration
```
