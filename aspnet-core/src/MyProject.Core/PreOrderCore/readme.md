...
```csharp
Configuration.Authorization.Providers.Add<WMS_PreOrderDetailAuthorizationProvider>();
...
```csharp
public virtual DbSet<WMS_PreOrderDetail>  WMS_PreOrderDetails { get; set; }
...
{
   modelBuilder.ApplyConfiguration(new  WMS_PreOrderDetailMapper());
}
...
```csharp 
        ...
        protected void SetUserTablePrefix(ModelBuilder modelBuilder)
        {
            // 代码生成器生成的表实体配置全部添加到这里
             modelBuilder.ApplyConfiguration(new  WMS_PreOrderDetailMapper());  // 请将代码配置到这里 
        }
...
```
Add-Migration Add_WMS_PreOrderDetailEntity_Migration
```
