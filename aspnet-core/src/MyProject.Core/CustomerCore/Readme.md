...
```csharp
Configuration.Authorization.Providers.Add<CustomerUserMappingAuthorizationProvider>();
...
```csharp
public DbSet<CustomerUserMapping>  CustomerUserMappings { get; set; }
...
{
   modelBuilder.ApplyConfiguration(new CustomerUserMappingCfg());
 }
...
```
Add-Migration AddNewCustomerUserMappingEntity_Migration
```
...
生成的多语言内容
在MyProject.Core类库中对应实体下的[多语言配置](CustomerUserMappingdouyuyan.md)文件中。
