...
```csharp
Configuration.Authorization.Providers.Add<WMS_OrderAddressAuthorizationProvider>();
...
```csharp
public virtual DbSet<WMS_OrderAddress>  WMS_OrderAddresss { get; set; }
...
{
   modelBuilder.ApplyConfiguration(new  WMS_OrderAddressMapper());
}
...
```csharp 
        ...
        protected void SetUserTablePrefix(ModelBuilder modelBuilder)
        {
            // 代码生成器生成的表实体配置全部添加到这里
             modelBuilder.ApplyConfiguration(new  WMS_OrderAddressMapper());  // 请将代码配置到这里 
        }
...
```
Add-Migration Add_WMS_OrderAddressEntity_Migration
```
