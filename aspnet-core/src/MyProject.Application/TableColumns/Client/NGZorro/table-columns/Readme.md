...
@NgModule({
	providers: [
			//以下内容复制进去
			ApiServiceProxies.Table_ColumnsServiceProxy,
			//
			],
	})
...
```ts
{//	的菜单按钮
text:'Table_Columns',i18n:'Table_Columns',acl:null,icon:'iconfont icon-dashboard',link:'/app/main/tablecolumns',sort:99},

```

请注意：以上link的值 '/app/main/tablecolumns') 中的 main在是指模块名称为main.module.ts，所以在添加菜单时请注意。
...
new MenuItem('Table_Columns', '', 'iconfont icon-dashboard', '/app/main/tablecolumns')
...
选择以下代码段,二选一
 //包含权限验证 
 ```
{  path: 'tablecolumns', component:Table_ColumnsComponent,data: { permission: 'Pages.Table_Columns' }  }, 

```
或
//不带权限验证
```
{  path: 'tablecolumns', component:Table_ColumnsComponent }, 
```
...
```
Table_ColumnsComponent,
CreateOrEditTable_ColumnsComponent,
