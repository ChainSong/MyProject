...
@NgModule({
	providers: [
			//以下内容复制进去
			ApiServiceProxies.WMS_ASNServiceProxy,
			//
			],
	})
...
```ts
{//	的菜单按钮
text:'WMS_ASN',i18n:'WMS_ASN',acl:'Pages.WMS_ASN',icon:'iconfont icon-dashboard',link:'/app/main/ASNCore',sort:99},

```

请注意：以上link的值 '/app/main/ASNCore') 中的 main在是指模块名称为main.module.ts，所以在添加菜单时请注意。
...
new MenuItem('WMS_ASN', 'Pages.WMS_ASN', 'iconfont icon-dashboard', '/app/main/ASNCore')
...
选择以下代码段,二选一
 //包含权限验证 
 ```
{  path: 'ASNCore', component:WMS_ASNComponent,data: { permission: 'Pages.WMS_ASN' }  }, 

```
或
//不带权限验证
```
{  path: 'ASNCore', component:WMS_ASNComponent }, 
```
