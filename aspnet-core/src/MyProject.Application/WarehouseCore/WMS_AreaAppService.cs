    ...
    public class WMS_AreaAppService : MyProjectAppServiceBase, IWMS_AreaAppService
    {
        ...
        public async Task<PagedResultDto<WMS_AreaListDto>> GetPaged(GetWMS_AreasInput input)
        {
            var query = _wms_areaRepository.GetAll()
                          ...
                          //模糊搜索 字段AreaName
                          .WhereIf(input.AreaStatus != 0, a => a.AreaStatus == (input.AreaStatus))
                          //模糊搜索 字段AreaName
                          .WhereIf(!input.AreaName.IsNullOrWhiteSpace(), a => a.AreaName.Contains(input.AreaName))
                          //模糊搜索 字段AreaType
                          .WhereIf(!input.AreaType.IsNullOrWhiteSpace(), a => a.AreaType.Contains(input.AreaType))
                          //模糊搜索 字段Remark
        ...
        /// <returns></returns>
        [HttpPost]
        public virtual async Task<List<SelectListItem>> GetAreaSelect(string input)
        {
            //TODO:新增前的逻辑判断，是否允许新增

            var query = _wms_areaRepository.GetAll()
                          //模糊搜索 字段WarehouseName
                          .WhereIf(!input.IsNullOrEmpty(), a => a.WarehouseName == (input))
            ;
            // TODO:根据传入的参数添加过滤条件
            ...
            var entityList = await query
                    ...
                    .Select(a => new SelectListItem { Label = a.AreaName, Value = a.Id.ToString() })
                    .ToListAsync();
