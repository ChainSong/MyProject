<template>
  <div>
    <Card dis-hover>
      ...
      <el-button icon="el-icon-search" type="primary" class="toolbar-btn" @click="exportReceipt">导出入库单</el-button>
      <el-button icon="el-icon-search" type="primary" class="toolbar-btn" @click="exportReceiptReceiving">导出上架库单</el-button>
        ...
        <el-table :data="list" ref="multipleTable" show-overflow-tooltip style="width:auto">
          ...
          <el-table-column fixed="right" label="操作" width="200">
            <template slot-scope="scope">
              ...
              <!-- <el-button @click="handleEdit(scope.row)" class="el-icon-edit" v-if="isGranted('Pages.Receipt.Edit')"
                type="text" size="small">上架</el-button> -->
    ...
    <receipt-query :close-on-click-modal="false" v-model="queryModalShow" @save-success="getpage"></receipt-query>
...
@Component({
  components: { selectTool, receiptCreate, receiptQuery },
})
export default class ReceiptList extends AbpBase {
  ...
  async handleDelete(row) {
    await this.$store.dispatch({ type: "receipt/delete", data: row }).then((res) => {
      console.log(res);
  ...
  handleQuery(row) {
    console.log(this.queryModalShow);
  ...
  exportReceiptReceiving() {
    ...
    this.$store.dispatch({
      type: "receipt/exportReceiptReceiving",
      data: ids,
    })
