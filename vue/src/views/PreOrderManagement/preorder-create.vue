...
export default class PreorderCreate extends AbpBase {
  ...
  //给上传组件赋值url
  uploadURL = url + "api/services/app/WMS_PreOrder/UploadExcelFile";
  //给上传组件赋值token
  ...
  GetImportExcelTemplate() {
    ...
    this.$store
      .dispatch({
        ...
        data: { CustomerId: this.header.customerId, TableName: "WMS_PreOrder" },
      })
      .then((res) => {
          ...
          if (valid) {
            await this.$store.dispatch({
              ...
              data: {
                ...
                // WMS_PreOrderDetails: this.details.line,
              },
            });
            (this.$refs.header as any).resetFields();
            this.details = { line: [new Detail()] };
