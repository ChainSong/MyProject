...
export default class selecttool extends AbpBase {
  //字段的值
  @Prop({ type: Object, default: new TableColumns() }) objData: Object;
  //本身的值
  ...
  @Prop({ type: Object, default: 0 }) objWhere: Object;

  // @Prop({ type: String, default: "" }) warehouseId: string;
  ...
  @Watch("objWhere", { immediate: true, deep: true })
  getObjWhere() {
  ...
  getDropDownListRemote() {
    ...
    if (this.objWhere != 0) {
      this.selectListItem.cstomerId = this.objWhere.cstomerId as number;
      this.selectListItem.warehouseId = this.objWhere.warehouseId as number;
    }
    this.options = [];
    this.$store
      .dispatch({
  ...
  getDropDownListRemoteData(query) {
    ...
    console.log(this.objWhere);
    if (this.objWhere != 0 && this.objWhere["customerId"] != undefined) {
      this.selectListItem.customerId = this.objWhere["customerId"] as number;
    }
    if (this.objWhere != 0 && this.objWhere["customerId"] != undefined) {
      this.selectListItem.warehouseId = this.objWhere["warehouseId"] as number;
    }
