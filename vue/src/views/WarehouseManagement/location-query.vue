<template>
  <div>
    <Modal title="库位详情" :value="value" width="50" @on-visible-change="visibleChange">
...
import Header from "../../store/entities/location";
// import Detail from "../../store/entities/locationDetail";
import TableColumns from "@/store/entities/tableColumns";
@Component
export default class LocationQuery extends AbpBase {
  ...
  gettableColumn() {
    this.tableColumnHeader.tableName = "WMS_Location";
  ...
  visibleChange(value: boolean) {
    if (!value) {
      ...
      this.header = Util.extend(
        true,
        {},
        this.$store.state.location.queryLocation
      );
      this.get();
    }
  }
