<template>
  <div>
    <Card dis-hover>
      <el-collapse accordion>
              ...
              <Row>
                ...
                <FormItem :label="i.displayName" v-if="i.isSearchCondition" style="width:95%;height: 30px;">
                  <template v-if="i.type == 'TextBox'">
                    <el-input placeholder="请输入内容" size="small" v-model="area[i.columnName]" v-if="i.isSearchCondition">
                    </el-input>
                  </template>
                  <template v-if="i.type == 'DropDownListInt'">
                    <el-select v-model="area[i.columnName]" size="small" v-if="i.isSearchCondition"
                  ...
                  <template v-if="i.type == 'DropDownListStr'">
                    <el-select v-model="area[i.columnName]" size="small" v-if="i.isSearchCondition" style="width:100%"
                      placeholder="请选择">
                  ...
                  <template v-if="i.type == 'DatePicker'">
                    <el-date-picker v-model="area[i.columnName]" type="daterange" v-if="i.isSearchCondition"
                      range-separator="~" start-placeholder="开始日期" end-placeholder="结束日期" style="width: 95%">
                    </el-date-picker>
                  </template>
                  <template v-if="i.type == 'DateTimePicker'" span="12">
                    <el-date-picker v-model="area[i.columnName]" v-if="i.isSearchCondition" type="datetimerange"
                      range-separator="~" start-placeholder="开始日期" end-placeholder="结束日期" style="width: 95%">
                    </el-date-picker>
                  </template>
                </FormItem>
       ...
       <el-button icon="el-icon-search" type="primary"   class="toolbar-btn" @click="getpage">{{ L("Find")
                ...
                <el-button type="primary"   class="toolbar-btn"  v-if="isGranted('Pages.Area.Create')"  @click="create" icon="el-icon-plus">{{ L("Add")
                }}</el-button>
        ...
        <el-table :data="list" style="width: 100%">
          ...
          <el-table-column fixed="right" label="操作" width="200">
            <template slot-scope="scope">
              ...
              <el-button @click="handleEdit(scope.row)"  v-if="isGranted('Pages.Area.Delete')" class="el-icon-edit" type="text" size="small">编辑</el-button>
              <el-popconfirm confirm-button-text="确定" v-if="isGranted('Pages.Area.Delete')" cancel-button-text="取消" icon="el-icon-info" icon-color="red"
    ...
    <area-create v-model="createModalShow" @save-success="getpage"></area-create>
    <area-edit v-model="editModalShow" @save-success="getpage"></area-edit>
    <area-query v-model="queryModalShow" @save-success="getpage"></area-query>
  </div>
</template>
...
import area from "@/store/entities/area";
import TableColumns from "../../store/entities/tableColumns";
import areaCreate from "./area-create.vue";
import areaEdit from "./area-edit.vue";
import areaQuery from "./area-query.vue";
import selectTool from "../Tool/select-tool.vue";
@Component({
  components: { areaCreate, areaEdit, areaQuery, selectTool },
})
export default class areaList extends AbpBase {
  tableColumn: TableColumns = new TableColumns();
  area: area = new area();
  tableColumns: Array<TableColumns> = new Array<TableColumns>();
  customers: Array<area> = new Array<area>();
  ...
  columns = [];
  get list() {
    console.log("this.$store.state.area.list");
    console.log(this.$store.state.area.list);
  ...
  get pageSize() {
    return this.$store.state.area.pageSize;
  }
  get totalCount() {
    return this.$store.state.area.totalCount;
  }
  get currentPage() {
    return this.$store.state.area.currentPage;
  }
  create() {
  ...
  getChildrenVal(column, label, relationColumn, value) {
    this.area[column] = label;
    this.area[relationColumn] = value;
  }
  //判断按钮权限
  isGranted(Granted) {
    // console.log(Granted);
  ...
  //编辑
  handleEdit(row) {
    this.$store.commit("area/edit", row);
    this.editModalShow = true;
  }
  //删除
  async handleDelete(row) {
    await this.$store.dispatch({ type: "area/delete", data: row });
    await this.getpage();
  }
  //查询
  handleQuery(row) {
    this.$store.commit("area/query", row);
    this.queryModalShow = true;
  }
  async getpage() {
    console.log("this.area")
    console.log(this.area)
    this.area.maxResultCount = this.pageSize;
    this.area.skipCount = (this.currentPage - 1) * this.pageSize;
  ...
  pageChange(page: number) {
    this.$store.commit("area/setCurrentPage", page);
    this.getpage();
  }
  pagesizeChange(pagesize: number) {
    this.$store.commit("area/setPageSize", pagesize);
    this.getpage();
  }
  gettableColumn() {
    this.tableColumn.tableName = "WMS_Area";
    this.$store
      .dispatch({
