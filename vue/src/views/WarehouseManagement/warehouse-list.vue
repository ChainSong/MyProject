      ...
      <div class="page-body">
        <Form ref="queryForm" :label-width="90" label-position="left" inline>
          <Row>
            ...
            <FormItem :label="i.displayName" v-if="i.isSearchCondition" style="width: 95%">
              <template v-if="i.type == 'TextBox'">
                <el-input placeholder="请输入内容" v-model="warehouse[i.columnName]" v-if="i.isSearchCondition">
                </el-input>
              </template>
              <template v-if="i.type == 'DropDownListInt'">
                <el-select v-model="warehouse[i.columnName]" v-if="i.isSearchCondition" placeholder="请选择">
          ...
          <Row>
            ...
            <Button type="primary" size="large" class="toolbar-btn" v-if="isGranted('Pages.Warehouse.Create')" @click="create" icon="android-add">{{ L("Add")
            }}</Button>
          </Row>
        </Form>
      </div>
        ...
        <el-table :data="list" style="width: 100%" max-height="300">
          ...
          <el-table-column fixed="right" label="操作" width="130">
            <template slot-scope="scope">
              ...
              <el-button @click="handleEdit(scope.row)" v-if="isGranted('Pages.Warehouse.Edit')" type="text" size="small">编辑</el-button>
              <el-popconfirm confirm-button-text="确定" v-if="isGranted('Pages.Warehouse.Delete')" cancel-button-text="取消" icon="el-icon-info" icon-color="red"
