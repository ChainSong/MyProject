<template>
  <div>
    <Modal title="出库单详情" :value="value" width="60" @on-visible-change="visibleChange">
      <!-- <el-container>
        <el-main> -->
          ...
          <el-descriptions-item v-bind:key="i.id" :prop="i.displayName" v-if="i.isCreate == 1 || i.isKey == 1">
            ...
            <template v-else-if="i.type == 'DropDownListInt'">
              <el-tag size="medium" v-for="item in i.tableColumnsDetails"
                    v-if="item.codeInt ==  header[i.columnName]" v-bind:key="item.codeStr" show-icon
                    :type="item.color">
                    {{ item.name }}
                  </el-tag>
              <!-- <template v-for="DropDown in i.tableColumnsDetails">
      ...
      <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="出库明细详情" name="出库明细详情">
          <template>
        ...
        <el-tab-pane label="出库分配详情" name="出库分配详情">

          <template>
            <el-form :model="orderAllocationData" ref="editDetail">
              <el-table :data="orderAllocationData.details" size="small" height="250" show-overflow-tooltip>
                <template v-for="(v, index) in orderAllocations">
                  <el-table-column v-if="v.type == 'DatePicker' && v.isCreate" v-bind:key="v.id" :fixed="false"
                    :prop="v.columnName" :label="v.displayName" min-width="100" max-height="50">
                    <template slot-scope="scope">
                      {{getformatDate(orderAllocationData.details[scope.$index][v.columnName])}}
                    </template>
                  </el-table-column>
                  <el-table-column v-else-if="v.type == 'DateTimePicker' && v.isCreate" v-bind:key="v.id" :fixed="false"
                    :prop="v.columnName" :label="v.displayName" min-width="100" max-height="50">
                    <template slot-scope="scope">
                      {{getformatTime(orderAllocationData.details[scope.$index][v.columnName])}}
                    </template>
                  </el-table-column>
                  <el-table-column v-else-if="v.isCreate" :key="index" :fixed="false" :label="v.displayName"
                    width="120">
                    <template slot-scope="scope">
                      <label v-text="orderAllocationData.details[scope.$index][v.columnName]"></label>
                    </template>
                  </el-table-column>
                </template>
              </el-table>
...
@Component
export default class OrderQuery extends AbpBase {
  ...
  orderAllocation: TableColumns = new TableColumns();
  orderAllocations: Array<TableColumns> = new Array<TableColumns>();
  ...
  orderAllocationData = { line: null, details: [new Detail()] };
  activeName: string='出库明细详情';
  created() {
    this.gettableColumn();
  }
  ...
  gettableColumn() {
    this.tableColumnHeader.tableName = "WMS_Order";
    this.tableColumnDetail.tableName = "WMS_OrderDetail";
    this.orderAllocation.tableName = "WMS_OrderAllocation";
    ...
    this.$store
      ...
      .dispatch({
        type: "tableColumns/GetByTableNameList",
        data: this.orderAllocation,
      })
      .then((res) => {
        console.log(res);
        this.orderAllocations = JSON.parse(
          localStorage.getItem(this.orderAllocation.tableName)
        ) as Array<TableColumns>; 
      })
      .catch((err) => {
        console.log(err);
  ...
  get() {
    this.$store
      ...
      .then((res) => {
        ...
        this.detailDatas.details = res.orderDetails;
  ...
  visibleChange(value: boolean) {
    ...
    if (!value) {
      ...
      this.header = Util.extend(
        ...
        this.$store.state.order.queryOrder
      );
      // console.log(this.header);
      this.get();
    }
