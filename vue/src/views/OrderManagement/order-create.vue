...
@Component({
  components: { selectTool, headerTool },
})
export default class preorderCreate extends AbpBase {
  @Prop({ type: Boolean, default: false }) value: boolean;
  //给上传组件赋值url
  uploadURL = url + "api/services/app/WMS_Order/UploadExcelFile";
  //给上传组件赋值token
  ...
  GetImportExcelTemplate() {
    ...
    this.$store
      .dispatch({
        ...
        data: { CustomerId: this.header.customerId, TableName: "WMS_Order" },
      })
      .then((res) => {
        ...
        (this.$refs.details as any).validate(async (valid: boolean) => {
          if (valid) {
            await this.$store.dispatch({
              type: "preorder/create",
              data: {
                WMS_Order: this.header,
                // WMS_OrderDetails: this.details.line,
              },
            });
            (this.$refs.header as any).resetFields();
            this.details = { line: [new Detail()] };
  ...
  ImportExcel(fileName: string) {
    ...
    this.$store
      .dispatch({
        type: "preorder/importExcel",
        data: { CustomerId: this.header.customerId, fileName: fileName },
      })
      .then((res) => {
